...
dat_attend_orig_exploded <- dat_attend_orig %>%
  nest(-PID) %>%
...
dat_attend_offspring <- d31 %>%
  ...
  separate(exam, c('label','exam_no'), sep = 6, convert = T) %>% select(-label) %>%
  filter(!is.na(yr)) %>%
  nest(exam_no, yr) %>%
  mutate(start_yr = map_dbl(data, ~min(.$yr))) %>%
...
dat_attend_offspring <- dat_attend_offspring %>%
  ...
  filter((is.na(YrFrac) | (YrFrac > start_yr))) %>%
  distinct()

dat_attend_offspring_exploded <- dat_attend_offspring %>%
  nest(-PID) %>%
